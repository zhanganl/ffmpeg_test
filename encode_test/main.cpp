...
int main(int argc, char* argv[])
{
	...
	char filename_in[] = "../parser_y.yuv";// "../ds_480x272.yuv";
	...
	//pFrame->linesize = ff_enc.pCodecCtx->width;
	...
	for (i = 0; i < framenum; i++) {
		...
		pFrame->pts = i;
		/* encode the image */
		ff_enc.encode(out_264, out_264_len, pFrame, got_output);
		...
		printf("count :%d\n",i);
		printf("******************out_264_len :%d********************\n", out_264_len);
		int j;
		if (got_output){
			for (j = 0; j < out_264_len; j++)
			{
				printf("%d  ", out_264[j]);
			}
			printf("out_264_len :%d\n", out_264_len);
	...
	//	ret = ff_enc.encode(out_264, out_264_len, NULL, got_output);
	//	if (ret < 0) {
	//		printf("Error encoding frame\n");
	//		return -1;
	//	}
